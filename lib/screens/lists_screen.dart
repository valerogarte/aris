...
class _ListsScreenState extends State<ListsScreen> {
  ...
  Future<void> _reloadListsFromStore() async {
    try {
      ...
      setState(() {
        ...
        _selectedListId = _resolveSelectedListId(
          current: _selectedListId,
          lists: _lists,
  ...
  String _resolveSelectedListId({
    ...
    required List<SubscriptionList> lists,
    required Map<String, Set<String>> assignments,
    required List<YouTubeSubscription> subscriptions,
  }) {
    ...
    final unassignedCount =
        subscriptions.length - assignedInSubscriptions.length;
    ...
    if (current != _allListId &&
        current != _unassignedListId &&
        !lists.any((list) => list.id == current)) {
      return _allListId;
    }
                ...
                child: Column(
                  ...
                  children: [
                    ListTile(
                      ...
                      subtitle: const Text('Asignar a listas'),
                    ),
                    if (_lists.isEmpty)
                      const Padding(
                        padding: EdgeInsets.all(24),
                        child: Text(
                          'No hay listas. Crea una lista en Perfil para empezar.',
                        ),
                      )
                    else
                      Flexible(
  ...
  Widget build(BuildContext context) {
    ...
    if (_subscriptions.isEmpty) {
      return const Center(
        child: Text('No hay suscripciones disponibles.'),
      );
    }
    ...
    for (final subscription in _subscriptions) {
      ...
      for (final listId in listIds) {
        listCountsInSubscriptions[listId] =
            (listCountsInSubscriptions[listId] ?? 0) + 1;
      }
    }

    final selectedAssignments = _selectedListId == _allListId ||
            _selectedListId == _unassignedListId
        ? null
        : (_assignments[_selectedListId] ?? <String>{});
    final listFiltered = _selectedListId == _allListId
        ...
        : _selectedListId == _unassignedListId
            ? _subscriptions
                .where(
                  (subscription) =>
                      !assignedChannels.contains(subscription.channelId),
                )
                .toList()
            : _subscriptions
                .where(
                  (subscription) =>
                      selectedAssignments!.contains(subscription.channelId),
                )
                .toList();
    final query = _searchQuery.trim().toLowerCase();
    final filteredSubscriptions = query.isEmpty
        ? listFiltered
        : listFiltered
            .where(
              (subscription) =>
                  subscription.title.toLowerCase().contains(query),
            )
            .toList();
    final unassignedCount =
        _subscriptions.length - assignedInSubscriptions.length;
    ...
    return RefreshIndicator(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          final listIds = _listsForChannel(subscription.channelId);
          final listItems =
              _lists.where((list) => listIds.contains(list.id)).toList();
          return ListTile(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return ChoiceChip(
            ...
            selectedColor: const Color(0xFFFA1021),
            side: BorderSide(
              color: selected ? const Color(0xFFFA1021) : Colors.transparent,
            ),
          );
        },
      ),
